...
class CGI {
 public:
  CGI(int fd) {
    fd_ = fd;
	script_ = "/Users/ramoussa/projects/letsgogeeky/webserv/webserv/hello.py";
  ...
  void wait() {
	// TODO: make this non-blocking and check if timeout reached return 408 error with timeout message
	int status;
	waitpid(pid_, &status, 0); // change to WNOHANG to make it non-blocking and update relevant code
	if (WIFEXITED(status)) {
	  ...
	  if (sent < strlen(HTTP_RESPONSE_CGI)) {
		// return 500 error
