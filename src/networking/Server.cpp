...
Server::Server(int port, const std::string& address)
    : serverSocket_(AF_INET, SOCK_STREAM, 0) {
...
void Server::run() {
  ...
  while (true) {
    int pollCount =
        pollManager_.pollSockets(250000);  // Use a reasonable timeout value
    if (pollCount == -1) {
      perror("poll error");
      break;
    }
    ...
    for (size_t i = 0; i < pollManager_.getPollSize(); ++i) {
      ...
      if (poll.revents & POLLIN) {
        if (poll.fd == serverSocket_.getFd()) {
          ...
          handleClientData(poll.fd);
        }
      } else if (poll.revents !=
                 0) {  // Handle other poll events (errors, hangups, etc.)
...
void Server::handleNewConnection() {
  try {
    ...
    clientConnections_[clientSocket.getFd()] =
        true;  // Mark the client as connected
    std::cout << "Accepted a new connection with fd: " << clientSocket.getFd()
              << std::endl;
...
void Server::handleClientData(int clientFd) {
  ...
  if (bytesRead <= 0) {
    ...
    buffer[bytesRead] = '\0';
    std::cout << "Received data from fd " << clientFd << ": " << buffer
              << std::endl;
    ...
    if (sentBytes == -1) {
      if (errno != EAGAIN && errno != EWOULDBLOCK) {
        perror("send error");
        clientConnections_[clientFd] =
            false;  // Mark as disconnected on send error
      } else {
...
void Server::handlePollError(int fd, const std::string& errorMessage) {
  std::cerr << errorMessage << " on fd: " << fd << " - " << strerror(errno)
            << std::endl;
